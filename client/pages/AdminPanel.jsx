...
import {
  UserPlus, Users, Eye, EyeOff, ShieldCheck, ShieldOff,
  CheckCircle2, AlertCircle, RefreshCw, Trash2, KeyRound,
  Pencil, X, Check, Mail,
} from "lucide-react";
import {
  db, ref, get, set, update, remove, onValue, off,
  createRangerInFirebase, sendPasswordResetEmail, auth,
  DB_PATHS,
} from "@/lib/firebase";
...
// Per-ranger item with inline edit, delete confirmation, and actions
function RangerItem({ ranger }) {
  ...
  const handleToggleActive = async () => {
    setBusy(true);
    try {
      await set(ref(db, `${DB_PATHS.USERS}/${ranger.uid}/isActive`), !ranger.isActive);
      showFeedback("success", ranger.isActive ? "Account disabled." : "Account enabled.");
    } catch (e) {
  ...
  const handleSaveName = async () => {
    ...
    setBusy(true);
    try {
      await update(ref(db, `${DB_PATHS.USERS}/${ranger.uid}`), { displayName: editName.trim() });
  ...
  const handleResetPassword = async () => {
    setBusy(true);
    try {
      // Send reset email with custom action URL pointing to our password reset page
      const actionCodeSettings = {
        url: `${window.location.origin}/reset-password`,
        handleCodeInApp: true,
      };
      await sendPasswordResetEmail(auth, ranger.email, actionCodeSettings);
      showFeedback("success", `Password reset email sent to ${ranger.email}. Check their inbox.`);
    } catch (e) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      // in case the realtime listener hasn't caught up yet
    ...
    <li className="p-3 rounded-lg border border-secondary hover:bg-secondary/20 transition-colors">
          ...
          <div className="min-w-0">
            {editing ? (
              <div className="flex items-center gap-1">
                ...
                />
                <button onClick={handleSaveName} disabled={busy} className="p-0.5 text-safe hover:text-safe/80" title="Save"><Check className="w-3.5 h-3.5" /></button>
                <button onClick={() => { setEditing(false); setEditName(ranger.displayName || ""); }} className="p-0.5 text-muted-foreground hover:text-foreground" title="Cancel"><X className="w-3.5 h-3.5" /></button>
              </div>
            ) : (
              <p className="text-sm font-medium text-foreground truncate">{ranger.displayName || "—"}</p>
            )}
            <p className="text-xs text-muted-foreground truncate">{ranger.email}</p>
            <p className="text-xs text-muted-foreground">
              Added {ranger.createdAt ? new Date(ranger.createdAt).toLocaleDateString("en-PH") : "—"}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-1.5 shrink-0">
          <span className={`text-xs px-2 py-0.5 rounded-full font-bold ${ranger.isActive ? "bg-safe/20 text-safe" : "bg-danger/20 text-danger"}`}>
            {ranger.isActive ? "Active" : "Disabled"}
          </span>
          {!confirmDel && (
            <>
              <button onClick={() => { setEditing(true); setEditName(ranger.displayName || ""); }} disabled={busy} title="Edit name"
                className="p-1.5 rounded-lg hover:bg-secondary transition-colors text-muted-foreground hover:text-foreground">
                <Pencil className="w-3.5 h-3.5" />
              </button>
              <button onClick={handleResetPassword} disabled={busy} title="Send password reset email"
                className="p-1.5 rounded-lg hover:bg-secondary transition-colors text-muted-foreground hover:text-foreground">
                <KeyRound className="w-3.5 h-3.5" />
              </button>
              <button onClick={handleToggleActive} disabled={busy} title={ranger.isActive ? "Disable account" : "Enable account"}
                className="p-1.5 rounded-lg hover:bg-secondary transition-colors text-muted-foreground hover:text-foreground">
                {ranger.isActive ? <ShieldOff className="w-3.5 h-3.5" /> : <ShieldCheck className="w-3.5 h-3.5" />}
              </button>
              <button onClick={() => setConfirmDel(true)} disabled={busy} title="Delete account"
      ...
      {confirmDel && (
        <div className="mt-2 flex items-center gap-2 bg-danger/5 border border-danger/20 rounded-lg px-3 py-2">
          ...
          <p className="text-xs text-danger flex-1">Delete this ranger permanently?</p>
      ...
      {feedback && (
        <div className={`mt-2 flex items-center gap-1.5 text-xs px-2 py-1 rounded ${
          feedback.type === "success" ? "text-safe bg-safe/10" : "text-danger bg-danger/10"
        }`}>
...
export default function AdminPanel() {
  ...
  const validate = () => {
    ...
    if (!email.endsWith("@gmail.com")) return "Only Gmail accounts are allowed (e.g. name@gmail.com). This ensures password reset emails can be delivered.";
    if (RULES.some((r) => !r.test(form.password))) return "Password does not meet all requirements.";
  ...
  const handleCreate = async (e) => {
    ...
    setCreating(true);
    try {
      const uid = await createRangerInFirebase(form.email.trim(), form.password);
      ...
      setFormSuccess(`Ranger "${form.displayName.trim()}" created successfully.`);
          ...
          <div className="bg-white rounded-xl border border-secondary shadow-sm p-6">
            ...
            <form onSubmit={handleCreate} className="space-y-4" autoComplete="off">
              <div>
                ...
                <input
                  type="text" value={form.displayName} onChange={field("displayName")}
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="ranger.name@gmail.com" autoComplete="new-email"
                  className={`w-full px-3 py-2 rounded-lg border bg-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary ${
                    form.email && !form.email.trim().toLowerCase().endsWith("@gmail.com") && form.email.includes("@")
                      ? "border-danger" : "border-secondary"
                  }`}
                />
                <p className="text-xs text-muted-foreground mt-1 flex items-center gap-1">
                  <Mail className="w-3 h-3" />
                  Gmail accounts only — required for password reset functionality.
                </p>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    type={showPw ? "text" : "password"} value={form.password} onChange={field("password")}
                ...
                {form.password && strength && (
                  <div className="mt-2">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">Strength: <span className="font-semibold">{strength.label}</span></p>
                  </div>
                )}
                ...
                {form.password && (
                  <ul className="mt-2 space-y-0.5">
                    {RULES.map((r) => {
                      ...
                      return (
                        <li key={r.id} className={`flex items-center gap-1.5 text-xs ${ok ? "text-safe" : "text-muted-foreground"}`}>
              ...
              <div>
                ...
                <input
                  type="password" value={form.confirm} onChange={field("confirm")}
                  placeholder="••••••••" autoComplete="new-password"
                  className={`w-full px-3 py-2 rounded-lg border bg-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary ${
                    form.confirm && form.confirm !== form.password ? "border-danger" : "border-secondary"
                  }`}
                />
          ...
          <div className="bg-white rounded-xl border border-secondary shadow-sm p-6">
            ...
            {listError && (
              <div className="bg-danger/10 border border-danger/30 rounded-lg p-3 text-sm text-danger mb-4">
                ...
                <p className="text-xs mt-1 opacity-80">Make sure Firebase security rules are deployed.</p>
              </div>
            )}
            ...
            {loadingList ? (
              ...
              <p className="text-muted-foreground text-sm py-4">No ranger accounts yet.</p>
            ) : (
            ...
            {rangers.length > 0 && (
              <div className="mt-4 pt-3 border-t border-secondary">
                <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
                  <span className="flex items-center gap-1"><Pencil className="w-3 h-3" /> Edit Name</span>
                  <span className="flex items-center gap-1"><KeyRound className="w-3 h-3" /> Reset Password</span>
                  <span className="flex items-center gap-1"><ShieldOff className="w-3 h-3" /> Toggle Active</span>
