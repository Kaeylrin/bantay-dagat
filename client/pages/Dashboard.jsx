            ...
            {latestReading && banner ? (
              ...
              >
                <div className="flex items-start gap-3">
                  <banner.icon
                    className={`w-8 h-8 ${banner.textClass} shrink-0 mt-0.5`}
                  />
                  <div>
                    <h2
                      className={`text-lg sm:text-xl font-header font-bold mb-1 ${banner.textClass}`}
                    >
                      {banner.title}
                    </h2>
                    <p
                      className={`text-xs sm:text-sm font-medium ${banner.textClass} opacity-90`}
                    >
                      {banner.desc}
                    </p>
                  </div>
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3 sm:gap-4">
            {sensors.map((sensor) => {
              ...
              // Sparklines always show last 20 for visual clarity on small cards
              ...
              return (
                ...
                >
                  ...
                  {/* Sensor name + description toggle */}
                  ...
                  {/* Expandable description + dynamic live status explanation */}
                  {isExpanded && (
                    <div className="text-[10px] mb-2 leading-relaxed border-l-2 border-primary/50 pl-2.5 py-1 bg-secondary/20 rounded-r-lg space-y-1">
                      <p className="font-semibold text-foreground">
                        {sensor.value !== null && sensor.value !== undefined ? (
                          <>
                            ...
                            >
                              {sensor.status === "safe"
                                ? `within the safe threshold (${sensor.threshold.safe[0]}–${sensor.threshold.safe[1]} ${sensor.unit})`
                                : sensor.status === "caution"
                                  ? `within the caution range (${sensor.threshold.caution[0]}–${sensor.threshold.caution[1]} ${sensor.unit})`
                                  : `in critical danger range!`}
                            </span>.
                          </>
                        ) : (
                          "Awaiting live sensor reading..."
                        )}
                      </p>
                  ...
                  {/* Per-sensor alert status contribution */}
                  <p className={`text-[10px] font-bold mb-2 ${alert.cls}`}>
                    {alert.text}
                  </p>
